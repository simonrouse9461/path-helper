function fish_path_helper -d "Generate PATH configuration commands"
    ...
    for file in $argv[2] $argv[2].d/*
      ...
      for line in (cat $file ^ /dev/null | grep -v '^\s*#')
        ...
        test -d $expanded_line
          ...
          or test -n "$argv[4]"
          or echo (set_color red)"error in file $file: $line is not a valid directory!"(set_color normal) 1>&2
      end
    end
  end
  ...
  set -q _flag_g
    and __generate_code PATH /etc/paths "$_flag_v" "$_flag_s"
    and __generate_code MANPATH /etc/manpaths "$_flag_v" "$_flag_s"
  set -q _flag_l
    and __generate_code PATH {$config_home}/paths "$_flag_v" "$_flag_s"
    and __generate_code MANPATH {$config_home}/manpaths "$_flag_v" "$_flag_s"
